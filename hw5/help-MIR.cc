...
void DataPath( const SD &coord,
               const Signal &mir, const Signal &mdr,
...
void simnet()
{
   ...
   SigV(mdr, 16, 0b1111000011111111);  // SigV() = Sig() with initialization
   Sig( Phase, 4 );
   ...
   DataPath("ih-kk", 
      ...
      ABus, BBus, A_Buff, B_Buff, Amux, Alu, C_bus, N, Z);
